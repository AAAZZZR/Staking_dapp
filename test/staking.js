...
describe("Staking", function () {
  ...
  describe("deploy", function () {
    ...
    it("set up tiers and lockPeriod", async function () {
  ...
  describe("stakeEther", function () {
    it("transfer ether", async function () {
      ...
      const receipt = await transaction.wait(); // wait until transaction complete
      const gasUsed = receipt.gasUsed.mul(receipt.effectiveGasPrice); //signer gas decrease stake Ether + gas

      //test change in signer1's balance
      ...
      //test change in contract balance
    });

    it("add a position to positions", async function () {
      const provider = waffle.provider;
      let positionl;
  ...
  describe("modifyLockPeriods", function () {
    describe("owner", function () {
      ...
      //currently exist 30,90,180
      it("should modify an exsiting lock period", async function () {
  ...
  describe("getPosionIdForAddress", function () {
    it("returns a list of positionId created by a specific address", async () => {
      let data;
      let transaction;
      ...
      const positionIds = await staking.getPosionIdForAddress(signer1.address);
  ...
  describe("changeUnlockDate", function () {
    describe("owner", function () {
      it("changes the unlockDate", async () => {
        ...
        const positionOId = await staking.getPositionById(0);

        const newUnLockDate = positionOId.unlockDate - 86400 * 500;
        ...
        expect(positionNew.unlockDate).to.be.equal(
          positionOId.unlockDate - 86400 * 500
        );
      });
    });
    describe("non owner", function () {
      it("reverts", async () => {
        ...
        ).to.be.revertedWith("only owner could modify it");
      });
    });
  });
  ...
  describe("closePosition",function(){
    describe('after unlock date', function() { 
        it("transfer principal and interest", async () =>{
            ...
            const newUnlockDate = block.timestamp - (86500 * 100)
            await staking.connect(signer1).changeUnlockDate(0,newUnlockDate)
     ...
     describe('before unlock date', function() { 
        it("transfer only principal", async () =>{
            let transaction;
            let receipt;
            let block;
            const provider = waffle.provider;
